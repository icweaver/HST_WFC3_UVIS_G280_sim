...
# ╔═╡ c4aa7074-a592-405d-a508-ef6f2de2f483
using LaTeXStrings

# ╔═╡ fbadeecd-c592-456a-a8fc-897560231d94
using Makie.Colors: hex
...
begin
	...
	using CairoMakie
	using CSV, DataFrames, Glob, NaturalSort
	using Statistics
	using AlgebraOfGraphics: set_aog_theme!, firasans
	...
	update_theme!(
		Theme(
			...
			#Density = (cycle=[[:color, :strokecolor] => :color],),
			Text = (; cycle=:color, font=firasans("Light")),
...
# ╔═╡ e9011814-d154-4966-9d07-af0c8593a4e8
cube = Dict(
	split(basename(fpath), '.')[1] => CSV.read(fpath, DataFrame)
	for fpath ∈ glob("../data/*.txt")
) |> x -> sort(x; lt=natural, rev=true)

# ╔═╡ 0634ba3e-8064-4eec-a6db-a4d50a5a32d7
superscript(i) = join(Char(0x2070 + d) for d in reverse!(digits(i)))

# ╔═╡ ce3491fd-eb24-4631-8df5-4715ec552918
subscript(i) = join(Char(0x2080 + d) for d in reverse!(digits(i)))

# ╔═╡ ea4ffc7a-7231-4623-99fc-974efdbfab27
subscript(1256)

# ╔═╡ 5c0fe2b2-bb6d-4176-85dd-36d99ae76f97
dist = cube["0_IMACS"].log_TiO

# ╔═╡ d1a371c8-5516-460b-a07b-1892a5ad8f66
f(x) = round(x; digits=1)

# ╔═╡ ed4cd21b-0896-461d-82f6-332874c667ae
print_d(d, d_u, d_d) = L"%$(f(d))^{+ %$(f(d_u))}_{- %$(f(d_d))}"

# ╔═╡ f68f0f1c-938f-49b3-8b16-c97a992a0872
function median_interval(dist; α=0.68)
	med = median(dist)
	upper, lower = quantile(dist, 0.5*(1.0 + α)), quantile(dist, 0.5*(1.0 - α))
	return med, upper-med, med-lower
end

# ╔═╡ d3376f81-7d51-4c81-9ae0-cc86c0ac2a16
d = median_interval(dist)

# ╔═╡ 279a10e3-ad5e-47e3-8c24-147e4137bbea
print_d(d...)

# ╔═╡ 852e4aaa-34ec-4722-9df9-e6fff2861b7e
...
# ╔═╡ 86a453b9-9053-423b-b651-a00822c32681
let
	fig = Figure()
	ax = Axis(fig[1, 1];
		xlabel = "log TiO",
		ylabel = "Density",
		limits = (-14, 0, 0, nothing),
	)
	
	for (i, (k, df)) ∈ enumerate(cube)
		dist = df.log_TiO
		d = median_interval(dist)
		...
		text!(ax, print_d(d...); position=(-10, 0.58 - i/20))
	end

	axislegend()

	savefig(fig, "abundance_TiO.pdf")
	
	fig
end

# ╔═╡ cf09fed1-85a4-4fa6-9460-fedd15010ed7
md"""
## Notebook setup 🔧
"""

# ╔═╡ 6a7e3c4f-8dcf-447d-9cf7-44ca14f8c64f
p = to_colormap(:YlOrBr_4, 8)

# ╔═╡ b54946c7-96ad-4787-b1a8-eec3079f41b2
p .|> hex

# ╔═╡ 630d236d-0b1d-4307-b6b0-c886636b8a6f
TableOfContents()
